...
class WD14Tagger:
    ...
    def INPUT_TYPES(s):
        ...
        return {"required": {
            ...
            "enabled": ("BOOLEAN", {"default": True}),  # 新增开关选项
        }}
    ...
    def tag(self, image, model, threshold, character_threshold, exclude_tags="", replace_underscore=False, trailing_comma=False, enabled=True):
        # 开关关闭时，返回空字符串
        if not enabled:
            return {"ui": {"tags": [""]}, "result": ([""],)}
        ...
        try:
            ...
            # 检查 tensor 是否为空
            if tensor.size == 0 or tensor.shape[0] == 0:
                return {"ui": {"tags": [""]}, "result": ([""],)}
            ...
            # 捕获任何异常，返回空字符串而不是报错
            print(f"[WD14Tagger] Error during tagging: {e}")
            return {"ui": {"tags": [""]}, "result": ([""],)}
